...
export default function MonthSection({ 
  ...
  const stickerTypes = ['smile', 'heart', 'star', 'flower', 'rainbow'] as const
  const doodleTypes = ['heart', 'star', 'swirl', 'arrow', 'circle'] as const
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        {photos.length > 2 && (
          ...
          >
            <Sticker type={stickerTypes[(index + 1) % stickerTypes.length]} size={40} />
          </div>
        )}
      </div>
        ...
        {photos.map((photo, photoIndex) => (
          <div key={photo.id} className="photo-wrapper">
            <PhotoCard
              ...
              tapePosition={photoIndex % 3 === 0 ? 'top' : photoIndex % 3 === 1 ? 'left' : 'right'}
            />
          </div>
        ))}
      </div>
